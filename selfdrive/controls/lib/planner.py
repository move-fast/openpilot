...
class Planner():
  ...
  def update(self, sm, pm, CP, VM, PP):
    ...
    if enabled and not self.first_loop and not sm['carState'].gasPressed:
      ...
      self.speed_limit_controller.ignore_solution()  # Flag speed limit controller to provide no solution.
    ...
    if enabled and (self.longitudinalPlanSource == 'model' or self.longitudinalPlanSource == 'limit'):
      print(f'{self.longitudinalPlanSource}, v_acc_start: {self.v_acc_start:.2f}, v_acc: {self.v_acc:.2f},\
         v_acc_future: {self.v_acc_future:.2f}, a_acc_start: {self.a_acc_start:.2f}, a_acc: {self.a_acc:.2f}')
